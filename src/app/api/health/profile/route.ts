...
export async function POST(request: Request) {
  ...
  if (conditions == null && familyHistory == null && investigations == null) {
    return NextResponse.json({ error: 'conditions, familyHistory and/or investigations (arrays) required' }, { status: 400 });
  }
  const profile = await prisma.healthProfile.update({
    ...
    select: { conditions: true, familyHistory: true, investigations: true },
  });
...
export async function GET() {
  ...
  return NextResponse.json(profile ?? { conditions: null, familyHistory: null, investigations: null });
}
